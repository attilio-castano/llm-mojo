...
def run_case(case_id: String, rows: Int, h: Int, i: Int) raises:
    var support = mlp_support()
    support.verify_case(case_id)
    var ctx = DeviceContext()
    assert_equal(ctx.api(), "metal")
    print("MLP runtime:", ctx.name(), ctx.api(), case_id)
    ...
    enqueue_mlp_stage_apple_gpu(ctx, weights, work, x, 0)
    check_mlp(work.normalized, case_id, "N", "local", 0, rows, h, cap * h)
    load_mlp(work.normalized, case_id + "/N.npy", 0, rows * h)
    enqueue_mlp_stage_apple_gpu(ctx, weights, work, x, 1)
    enqueue_mlp_stage_apple_gpu(ctx, weights, work, x, 2)
    ...
    enqueue_mlp_stage_apple_gpu(ctx, weights, work, x, 3)
