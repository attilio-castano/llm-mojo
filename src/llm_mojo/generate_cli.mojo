...
def main() raises:
    ...
    while offset < prompt_length:
        ...
        model.forward(ctx,ids,configuration,"",select_token_selection(args[6],rows,ctx.name()))
        if diagnostics:
    ...
    for step in range(budget):
        ...
        if step+1 < budget:
            ...
            model.forward(ctx,ids,select_configuration(args[6],1,model.length+1,ctx.name()),"",select_token_selection(args[6],1,ctx.name()))
            if diagnostics:
                ctx.synchronize()
