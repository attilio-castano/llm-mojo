...
def collect(source, output, prefill_variant=None, *, prefill_variants=None, prefix='', attention_sublayer=False, wo_comparison=False, decode_comparison=False, prefill_comparison=False, projection_comparison=False, parallelism_variants=None):
    records, samples = [], []
    common = None
    ...
    if (wo_comparison or decode_comparison or prefill_comparison or projection_comparison or parallelism_variants is not None) and not attention_sublayer:
        raise ValueError('contained comparison requires the attention sublayer')
    if sum((wo_comparison, decode_comparison, prefill_comparison, projection_comparison, parallelism_variants is not None)) > 1:
...
if __name__ == '__main__':
    ...
    collect(args.source, args.output, args.prefill_variant,
            ...
            parallelism_variants=args.parallelism_variants)
