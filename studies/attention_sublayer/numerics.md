# Attention precision investigation

Historical observations explain the selected policy. For the current gates, see the
[attention contract](../../docs/attention-sublayer.md). Failed compatibility checks
remain evidence; they are not passing gates for the selected FP32 policy.

## Historical BF16 compatibility contract
...
```sh
uv run --locked --script tests/fixtures/generate.py attention_sublayer -- --calibrate
...
uv run --locked --script tests/fixtures/generate.py attention_sublayer -- --holdout --compare-backends
...
```sh
uv run --locked --script tests/fixtures/generate.py attention_checkpoint -- --download --attention-prefix
...
The earlier [numerical record](data/numerics.json)
...
The [FP32 experiment record](data/precision_numerics.json)
